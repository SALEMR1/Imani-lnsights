// ملف أذكار المساء
document.addEventListener('DOMContentLoaded', function() {
    // بيانات أذكار المساء
    const eveningAzkar = [
        {
            text: "أَعُوذُ بِاللهِ مِنْ الشَّيْطَانِ الرَّجِيمِ\n\nاللهُ لَا إِلَٰهَ إِلَّا هُوَ الْحَيُّ الْقَيُّومُ لَا تَأْخُذُهُ سِنَةٌ وَلَا نَوْمٌ لَهُ مَا فِي السَّمَاوَاتِ وَمَا فِي الْأَرْضِ مَنْ ذَا الَّذِي يَشْفَعُ عِنْدَهُ إِلَّا بِإِذْنِهِ يَعْلَمُ مَا بَيْنَ أَيْدِيهِمْ وَمَا خَلْفَهُمْ وَلَا يُحِيطُونَ بِشَيْءٍ مِنْ عِلْمِهِ إِلَّا بِمَا شَاءَ وَسِعَ كُرْسِيُّهُ السَّمَاوَاتِ وَالْأَرْضَ وَلَا يَئُودُهُ حِفْظُهُمَا وَهُوَ الْعَلِيُّ الْعَظِيمُ",
            count: 1,
            reference: "من قالها حين يصبح أجير من الجن حتى يمسي، ومن قالها حين يمسي أجير من الجن حتى يصبح. [رواه الحاكم]"
        },
        {
            text: "قُلْ هُوَ اللَّهُ أَحَدٌ ۝ اللَّهُ الصَّمَدُ ۝ لَمْ يَلِدْ وَلَمْ يُولَدْ ۝ وَلَمْ يَكُنْ لَهُ كُفُوًا أَحَدٌ",
            count: 3,
            reference: "من قالها حين يصبح وحين يمسي ثلاث مرات كفته من كل شيء. [رواه أبو داود والترمذي]"
        },
        {
            text: "قُلْ أَعُوذُ بِرَبِّ الْفَلَقِ ۝ مِنْ شَرِّ مَا خَلَقَ ۝ وَمِنْ شَرِّ غَاسِقٍ إِذَا وَقَبَ ۝ وَمِنْ شَرِّ النَّفَّاثَاتِ فِي الْعُقَدِ ۝ وَمِنْ شَرِّ حَاسِدٍ إِذَا حَسَدَ",
            count: 3,
            reference: "من قالها حين يصبح وحين يمسي ثلاث مرات كفته من كل شيء. [رواه أبو داود والترمذي]"
        },
        {
            text: "قُلْ أَعُوذُ بِرَبِّ النَّاسِ ۝ مَلِكِ النَّاسِ ۝ إِلَٰهِ النَّاسِ ۝ مِنْ شَرِّ الْوَسْوَاسِ الْخَنَّاسِ ۝ الَّذِي يُوَسْوِسُ فِي صُدُورِ النَّاسِ ۝ مِنَ الْجِنَّةِ وَالنَّاسِ",
            count: 3,
            reference: "من قالها حين يصبح وحين يمسي ثلاث مرات كفته من كل شيء. [رواه أبو داود والترمذي]"
        },
        {
            text: "أَمْسَيْنَا وَأَمْسَى الْمُلْكُ لِلَّهِ، وَالْحَمْدُ لِلَّهِ، لَا إِلَٰهَ إِلَّا اللهُ وَحْدَهُ لَا شَرِيكَ لَهُ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ وَهُوَ عَلَى كُلِّ شَيْءٍ قَدِيرٌ. رَبِّ أَسْأَلُكَ خَيْرَ مَا فِي هَذِهِ اللَّيْلَةِ وَخَيْرَ مَا بَعْدَهَا، وَأَعُوذُ بِكَ مِنْ شَرِّ مَا فِي هَذِهِ اللَّيْلَةِ وَشَرِّ مَا بَعْدَهَا، رَبِّ أَعُوذُ بِكَ مِنَ الْكَسَلِ وَسُوءِ الْكِبَرِ، رَبِّ أَعُوذُ بِكَ مِنْ عَذَابٍ فِي النَّارِ وَعَذَابٍ فِي الْقَبْرِ",
            count: 1,
            reference: "رواه مسلم"
        },
        {
            text: "اللَّهُمَّ بِكَ أَمْسَيْنَا، وَبِكَ أَصْبَحْنَا، وَبِكَ نَحْيَا، وَبِكَ نَمُوتُ، وَإِلَيْكَ الْمَصِيرُ",
            count: 1,
            reference: "رواه الترمذي"
        },
        {
            text: "اللَّهُمَّ أَنْتَ رَبِّي لَا إِلَٰهَ إِلَّا أَنْتَ، خَلَقْتَنِي وَأَنَا عَبْدُكَ، وَأَنَا عَلَى عَهْدِكَ وَوَعْدِكَ مَا اسْتَطَعْتُ، أَعُوذُ بِكَ مِنْ شَرِّ مَا صَنَعْتُ، أَبُوءُ لَكَ بِنِعْمَتِكَ عَلَيَّ، وَأَبُوءُ بِذَنْبِي فَاغْفِرْ لِي فَإِنَّهُ لَا يَغْفِرُ الذُّنُوبَ إِلَّا أَنْتَ",
            count: 1,
            reference: "من قالها موقناً بها حين يمسي فمات من ليلته دخل الجنة، وكذلك حين يصبح. [رواه البخاري]"
        },
        {
            text: "اللَّهُمَّ إِنِّي أَمْسَيْتُ أُشْهِدُكَ، وَأُشْهِدُ حَمَلَةَ عَرْشِكَ، وَمَلَائِكَتَكَ، وَجَمِيعَ خَلْقِكَ، أَنَّكَ أَنْتَ اللهُ لَا إِلَٰهَ إِلَّا أَنْتَ وَحْدَكَ لَا شَرِيكَ لَكَ، وَأَنَّ مُحَمَّدًا عَبْدُكَ وَرَسُولُكَ",
            count: 4,
            reference: "من قالها حين يصبح أو يمسي أربع مرات أعتقه الله من النار. [رواه أبو داود]"
        },
        {
            text: "اللَّهُمَّ مَا أَمْسَى بِي مِنْ نِعْمَةٍ أَوْ بِأَحَدٍ مِنْ خَلْقِكَ فَمِنْكَ وَحْدَكَ لَا شَرِيكَ لَكَ، فَلَكَ الْحَمْدُ وَلَكَ الشُّكْرُ",
            count: 1,
            reference: "من قالها حين يصبح فقد أدى شكر يومه، ومن قالها حين يمسي فقد أدى شكر ليلته. [رواه أبو داود]"
        },
        {
            text: "اللَّهُمَّ عَافِنِي فِي بَدَنِي، اللَّهُمَّ عَافِنِي فِي سَمْعِي، اللَّهُمَّ عَافِنِي فِي بَصَرِي، لَا إِلَٰهَ إِلَّا أَنْتَ. اللَّهُمَّ إِنِّي أَعُوذُ بِكَ مِنَ الْكُفْرِ وَالْفَقْرِ، وَأَعُوذُ بِكَ مِنْ عَذَابِ الْقَبْرِ، لَا إِلَٰهَ إِلَّا أَنْتَ",
            count: 3,
            reference: "رواه أبو داود"
        },
        {
            text: "حَسْبِيَ اللهُ لَا إِلَٰهَ إِلَّا هُوَ عَلَيْهِ تَوَكَّلْتُ وَهُوَ رَبُّ الْعَرْشِ الْعَظِيمِ",
            count: 7,
            reference: "من قالها حين يصبح وحين يمسي سبع مرات كفاه الله ما أهمه من أمر الدنيا والآخرة. [رواه ابن السني]"
        },
        {
            text: "اللَّهُمَّ إِنِّي أَسْأَلُكَ الْعَافِيَةَ فِي الدُّنْيَا وَالْآخِرَةِ، اللَّهُمَّ إِنِّي أَسْأَلُكَ الْعَفْوَ وَالْعَافِيَةَ فِي دِينِي وَدُنْيَايَ وَأَهْلِي وَمَالِي، اللَّهُمَّ اسْتُرْ عَوْرَاتِي، وَآمِنْ رَوْعَاتِي، اللَّهُمَّ احْفَظْنِي مِنْ بَيْنِ يَدَيَّ، وَمِنْ خَلْفِي، وَعَنْ يَمِينِي، وَعَنْ شِمَالِي، وَمِنْ فَوْقِي، وَأَعُوذُ بِعَظَمَتِكَ أَنْ أُغْتَالَ مِنْ تَحْتِي",
            count: 1,
            reference: "رواه أبو داود وابن ماجه"
        },
        {
            text: "اللَّهُمَّ عَالِمَ الْغَيْبِ وَالشَّهَادَةِ فَاطِرَ السَّمَاوَاتِ وَالْأَرْضِ، رَبَّ كُلِّ شَيْءٍ وَمَلِيكَهُ، أَشْهَدُ أَنْ لَا إِلَٰهَ إِلَّا أَنْتَ، أَعُوذُ بِكَ مِنْ شَرِّ نَفْسِي، وَمِنْ شَرِّ الشَّيْطَانِ وَشِرْكِهِ، وَأَنْ أَقْتَرِفَ عَلَى نَفْسِي سُوءًا، أَوْ أَجُرَّهُ إِلَى مُسْلِمٍ",
            count: 1,
            reference: "رواه الترمذي وأبو داود"
        },
        {
            text: "بِسْمِ اللهِ الَّذِي لَا يَضُرُّ مَعَ اسْمِهِ شَيْءٌ فِي الْأَرْضِ وَلَا فِي السَّمَاءِ وَهُوَ السَّمِيعُ الْعَلِيمُ",
            count: 3,
            reference: "من قالها ثلاث مرات حين يصبح وثلاث مرات حين يمسي لم يضره شيء. [رواه أبو داود والترمذي وابن ماجه]"
        },
        {
            text: "رَضِيتُ بِاللهِ رَبًّا، وَبِالْإِسْلَامِ دِينًا، وَبِمُحَمَّدٍ صَلَّى اللهُ عَلَيْهِ وَسَلَّمَ نَبِيًّا",
            count: 3,
            reference: "من قالها حين يصبح وحين يمسي ثلاث مرات كان حقاً على الله أن يرضيه يوم القيامة. [رواه أحمد والترمذي وابن ماجه والنسائي]"
        },
        {
            text: "أَعُوذُ بِكَلِمَاتِ اللهِ التَّامَّاتِ مِنْ شَرِّ مَا خَلَقَ",
            count: 3,
            reference: "من قالها حين يمسي ثلاث مرات لم تضره حمة تلك الليلة. [رواه الترمذي وابن ماجه]"
        },
        {
            text: "اللَّهُمَّ إِنِّي أَعُوذُ بِكَ مِنَ الْهَمِّ وَالْحَزَنِ، وَالْعَجْزِ وَالْكَسَلِ، وَالْبُخْلِ وَالْجُبْنِ، وَضَلَعِ الدَّيْنِ، وَغَلَبَةِ الرِّجَالِ",
            count: 1,
            reference: "رواه البخاري"
        },
        {
            text: "أَسْتَغْفِرُ اللهَ الَّذِي لَا إِلَٰهَ إِلَّا هُوَ، الْحَيَّ الْقَيُّومَ، وَأَتُوبُ إِلَيْهِ",
            count: 3,
            reference: "من قالها حين يمسي غفرت ذنوبه وإن كان قد فر من الزحف. [رواه أبو داود والترمذي]"
        },
        {
            text: "يَا حَيُّ يَا قَيُّومُ بِرَحْمَتِكَ أَسْتَغِيثُ، أَصْلِحْ لِي شَأْنِي كُلَّهُ، وَلَا تَكِلْنِي إِلَى نَفْسِي طَرْفَةَ عَيْنٍ",
            count: 3,
            reference: "رواه الحاكم"
        },
        {
            text: "أَمْسَيْنَا وَأَمْسَى الْمُلْكُ لِلَّهِ رَبِّ الْعَالَمِينَ، اللَّهُمَّ إِنِّي أَسْأَلُكَ خَيْرَ هَذِهِ اللَّيْلَةِ: فَتْحَهَا، وَنَصْرَهَا، وَنُورَهَا، وَبَرَكَتَهَا، وَهُدَاهَا، وَأَعُوذُ بِكَ مِنْ شَرِّ مَا فِيهَا وَشَرِّ مَا بَعْدَهَا",
            count: 1,
            reference: "رواه أبو داود"
        },
        {
            text: "سُبْحَانَ اللهِ وَبِحَمْدِهِ",
            count: 100,
            reference: "من قالها مائة مرة حين يصبح وحين يمسي لم يأتِ أحد يوم القيامة بأفضل مما جاء به إلا أحد قال مثل ما قال أو زاد عليه. [رواه مسلم]"
        }
    ];

    // الحصول على عنصر حاوية أذكار المساء
    const eveningContainer = document.getElementById('evening-azkar');
    
    // التحقق من وجود الحاوية
    if (!eveningContainer) {
        console.error('لم يتم العثور على حاوية أذكار المساء');
        return;
    }
    
    // مسح المحتوى الحالي
    eveningContainer.innerHTML = '';
    
    // إضافة الأذكار إلى الحاوية
    eveningAzkar.forEach(zikr => {
        // إنشاء بطاقة الذكر
        const zikrCard = document.createElement('div');
        zikrCard.className = 'zikr-card';
        
        // إنشاء نص الذكر
        const zikrText = document.createElement('div');
        zikrText.className = 'zikr-text';
        
        // إنشاء فقرة النص
        const paragraph = document.createElement('p');
        paragraph.textContent = zikr.text;
        zikrText.appendChild(paragraph);
        
        // إنشاء قسم المصدر إذا كان موجوداً
        if (zikr.reference) {
            const reference = document.createElement('div');
            reference.className = 'zikr-reference';
            reference.textContent = zikr.reference;
            zikrText.appendChild(reference);
        }
        
        // إنشاء قسم العدد
        const zikrCount = document.createElement('div');
        zikrCount.className = 'zikr-count';
        
        // إنشاء نص العدد
        const countText = document.createElement('span');
        countText.textContent = zikr.count > 1 ? `${zikr.count} مرات` : 'مرة واحدة';
        zikrCount.appendChild(countText);
        
        // إنشاء زر العد
        const countButton = document.createElement('button');
        countButton.innerHTML = '<i class="fas fa-plus"></i>';
        
        // إضافة وظيفة العد
        let currentCount = 0;
        countButton.addEventListener('click', function() {
            if (currentCount < zikr.count) {
                currentCount++;
                countText.textContent = `${currentCount}/${zikr.count}`;
                
                // إذا اكتمل العدد، تغيير لون الزر
                if (currentCount === zikr.count) {
                    this.style.backgroundColor = '#84b026';
                }
            }
        });
        
        // إضافة زر العد إلى قسم العدد
        zikrCount.appendChild(countButton);
        
        // إضافة الأقسام إلى بطاقة الذكر
        zikrCard.appendChild(zikrText);
        zikrCard.appendChild(zikrCount);
        
        // إضافة بطاقة الذكر إلى الحاوية
        eveningContainer.appendChild(zikrCard);
    });
});
